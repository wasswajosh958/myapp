...
interface UtilityDao {
    @Query("SELECT * FROM utilities WHERE userId = :userId")
    fun getAllUtilities(userId: Int): Flow<List<UtilityEntity>>

    @Insert
    suspend fun insert(utility: UtilityEntity)

    @Update
    suspend fun update(utility: UtilityEntity)

    @Delete
    suspend fun delete(utility: UtilityEntity)

    @Query("DELETE FROM utilities WHERE userId = :userId")
    suspend fun deleteAll(userId: Int)
}
